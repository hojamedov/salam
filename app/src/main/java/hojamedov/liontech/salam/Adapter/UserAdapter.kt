...
import hojamedov.liontech.salam.Fragments.ProfileFragment
...
class UserAdapter(
    ...
    override fun onBindViewHolder(holder: UserAdapter.ViewHolder, position: Int) {
        ...
        holder.followButton.setOnClickListener {
            if (holder.followButton.text.toString() == "Yzarlamak") {
                firebaseUser?.uid.let { itl ->
                    FirebaseDatabase.getInstance().reference
                        .child("Yzarlamak").child(itl.toString())
                        .child("Yzarlanýar").child(user.getUID())
                        .setValue(true).addOnCompleteListener { task ->
                            if (task.isSuccessful) {
                                firebaseUser?.uid.let { itl ->
                                    FirebaseDatabase.getInstance().reference
                                        .child("Yzarlamak").child(user.getUID())
                                        .child("Yzarlaýanlar").child(it.toString())
                                        .setValue(true).addOnCompleteListener { task ->
                                            if (task.isSuccessful) {
        ...
        followingRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(datasnapshot: DataSnapshot) {
                if (datasnapshot.child(uid).exists()) {
                    followButton.text = "Yzarlanýar"
                } else {
                    followButton.text = "Yzarlamak"
                }
            }
